...
The concept of pair-programming is quite old. It consists in developing with \emph{two persons} on a \emph{single computer}.
One person is the \emph{observer}. He thinks ahead of the future problems that may occur and double checks
...
\begin{description}
    \item[Sharing knowledge] with colleagues. This is \emph{vital} to avoid having \emph{silos} in a team. It is important
        that every member should be replaceable. This is especially true when people are out of office.
...
Working remotely on that server restricts developers to use command line programs only because
...
When the code is ready, I push it via \gls{git} on \gls{gerrit} for review.
...
It is based on a notation system with in-line comments and its usage is pretty straightforward.
...
A lot of automatic and manual tests scenarios are run. They cover \emph{voice call}, \emph{voip call}, \emph{media playback}
...
Every task has a dedicated section in this document to describe what has been done.
...
\emph{business oriented} and focused on the client's needs. The demo time has gone from one hour and a half to less than an hour.
This helps us keeping things short and keeping off-topics away from the meeting.
I participated in every Demo and even showed some products as a member of the \gls{scrum} team.
