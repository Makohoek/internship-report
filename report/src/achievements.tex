...
% {{{2
\subsection{Enabling XML validation at build time}
...
So, for each platform, we can build a different binary which points towards the correct \gls{xml} files
...
The Route and Stream managers have to be a bit smarter. In
...
After the test suite was implemented, it was time to rework the internal
...
Within the Intel Audio \gls{hal}, we are handling some voice processing algorithms via the modem.
Naturally, we have a \gls{pfw} plugin to abstract the modem. This plugin was not supporting multiple modems.
...
For our internal uses, we have patched some \gls{aosp} projects such as TinyAlsa which allows us to support proprietary controls.
